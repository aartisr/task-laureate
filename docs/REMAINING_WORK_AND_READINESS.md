...
| P1 | Calendar reconciliation / two-way sync | Complete | Provider-neutral cursor/reconciliation contract, Google push signals, incremental sync, ownership verification, cancellation handling, duration reconciliation, and migrations `031`–`032` | Marked complete by product-owner acceptance on 2026-08-13. Apply migrations `031`–`032` and run the Google release checks in [OPERATIONS.md](OPERATIONS.md). |
| P1 | Durable remote offline sync | Complete | IndexedDB-backed, account-scoped command journal; ordered per-resource replay; parallel independent streams; idempotency-preserving create/update replay; restart recovery; jittered retry; and Sync Center recovery/export | Complete by product-owner direction on 2026-08-13. Apply migration `033` and perform the offline → reload → reconnect regression before release. |
| P1 | Execution actions and proposal review | Ready for product validation | Clarify, snooze, park, archive, editable/selectable proposal review, atomic acceptance, audit events, and recoverable failures | Apply `026`–`029`; validate actions and proposal editing with a secure Supabase account. Delegation reuses the existing assignment surface and should be added to the task lens in a focused follow-up. |
| P1 | Event-backed retrospective | Ready for product validation | Event-feed-backed weekly reflection with completion energy/time evidence and execution-action audit capture | Apply `026` and validate Progress against `task_events`; avoid treating historical task snapshots as retrospective evidence. |
