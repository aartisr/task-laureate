...
| P1 | Durable remote offline sync | Ready for product validation | IndexedDB-backed, account-scoped command journal; ordered per-resource replay; parallel independent streams; idempotency preservation; restart recovery; jittered retry; and an actionable sync/review surface | Validate offline → reconnect with a real Supabase account on Chrome, Safari, and mobile before product-owner acceptance. New-record creation remains deliberately server-confirmed until its idempotent create RPC is introduced. |
