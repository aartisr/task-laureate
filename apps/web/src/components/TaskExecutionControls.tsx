...
export function TaskExecutionControls({ task }: { task: TodoItem }) {
  ...
  return <section className="panel" aria-label="Execution planning"><div className="panel-heading"><div><p className="eyebrow">Make this feasible</p><h2>Plan and deconstruct</h2></div></div><div className="button-row"><button className="primary-button" type="button" onClick={() => { setSelectedSteps(proposal.steps.map((step) => step.title)); setProposalVisible((value) => !value); }}>Deconstruct task</button></div><details className="task-execution-controls__details"><summary>Planning details</summary><div className="button-row"><label className="field"><span>Minutes</span><input value={estimate} type="number" min="1" max="1440" onChange={(event) => setEstimate(event.target.value)} /></label><label className="field"><span>Energy</span><select value={energy} onChange={(event) => setEnergy(event.target.value as EnergyLevel)}><option value="deep">Deep focus</option><option value="light">Light energy</option><option value="quick">Quick win</option></select></label><button className="secondary-button" type="button" onClick={() => void savePlan()}>Save plan</button></div><div className="button-row"><button type="button" className="secondary-button" onClick={() => void snooze()}>Snooze to tomorrow</button><button type="button" className="secondary-button" onClick={() => void park()}>Park for review</button><button type="button" className="secondary-button" onClick={() => void archive()}>Archive</button></div></details>{proposalVisible ? <div className="card-list">{proposal.steps.map((step) => <label className="data-card" key={step.title}><input type="checkbox" checked={selectedSteps.includes(step.title)} onChange={() => setSelectedSteps((items) => items.includes(step.title) ? items.filter((item) => item !== step.title) : [...items, step.title])} /><div className="data-card__content"><strong>{step.title}</strong><p>{step.estimateMinutes} min · {step.energyLevel}</p></div></label>)}<div className="button-row"><button className="primary-button" type="button" disabled={!selectedSteps.length || mutations.createTask.isPending} onClick={() => void acceptSteps()}>Accept selected</button><button className="secondary-button" type="button" onClick={() => setProposalVisible(false)}>Discard</button></div></div> : null}{notice ? <p role="status">{notice}</p> : null}</section>;
}
