  ...
  useEffect(() => {
    ...
    void getTaskReminderSetup(taskId)
      ...
      .catch((error) => {
        if (!active) return;
        setSetupFailed(true);
        setSetupError(error instanceof Error ? error.message : 'Loading collaboration settings failed.');
      })
