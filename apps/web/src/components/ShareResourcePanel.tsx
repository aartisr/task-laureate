  ...
  return <div className="share-resource-panel" role="dialog" aria-modal="true" aria-labelledby={titleId}>
    ...
    <section className="share-resource-panel__content" ref={dialogRef} tabIndex={-1}>
      ...
      <form onSubmit={invite}>
        ...
        <fieldset disabled={!canManageSharing}><legend>What should they be able to do?</legend><div className="share-resource-panel__roles">
          {(['editor', 'viewer'] as const).map((candidate) => <button key={candidate} type="button" aria-pressed={role === candidate} onClick={() => setRole(candidate)}><strong>{candidate === 'editor' ? 'Help update work' : 'View only'}</strong><span>{candidate === 'editor' ? 'Can add, edit, and complete tasks. Cannot manage sharing or List settings.' : 'Can read tasks and notes. Cannot add, edit, complete, or delete work.'}</span></button>)}
        </div></fieldset>
        <p className="share-resource-panel__selection" role="status">They will receive <strong>{role === 'editor' ? 'editing access to tasks' : 'view-only access'}</strong> after they accept the invitation while signed in with this email.</p>
        <button className="primary-button" type="submit" disabled={sharing || !canManageSharing}>{sharing ? 'Creating invite…' : canManageSharing ? `Invite as ${role === 'editor' ? 'editor' : 'viewer'}` : 'Task owner required'}</button>
      </form>
