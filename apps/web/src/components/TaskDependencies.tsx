...
export function TaskDependencies({ taskId, listId, readOnly = false, onSummaryChange }: { taskId: string; listId: string; readOnly?: boolean; onSummaryChange?: (summary: TaskDependencySummary) => void }) {
  ...
  return <section className="task-dependencies" aria-labelledby={`dependencies-${taskId}`}>
    <header><div><p className="eyebrow">Dependency lens</p><h3 id={`dependencies-${taskId}`}>Work order</h3></div><span className={waitingOn.some((edge) => edge.required && edge.type === 'finish_to_start' && edge.prerequisiteTask.status !== 'done') ? 'task-dependencies__state is-blocked' : 'task-dependencies__state is-ready'}>{waitingOn.some((edge) => edge.required && edge.type === 'finish_to_start' && edge.prerequisiteTask.status !== 'done') ? `Blocked · ${waitingOn.filter((edge) => edge.required && edge.type === 'finish_to_start' && edge.prerequisiteTask.status !== 'done').length}` : 'Ready'}</span></header>
    <div className="task-dependencies__columns">
      <div><h4>Waiting on</h4>{waitingOn.length ? <ul>{waitingOn.map((edge) => <li key={edge.id}><span><b>{edge.prerequisiteTask.status === 'done' ? '✓' : '↳'}</b><strong>{edge.prerequisiteTask.title}</strong><small>{dependencyTypeLabel[edge.type]} · {edge.prerequisiteTask.status === 'done' ? 'Complete' : 'Open'}</small></span>{!readOnly ? <button type="button" onClick={() => void remove(edge)} disabled={saving} aria-label={`Remove dependency on ${edge.prerequisiteTask.title}`}>×</button> : null}</li>)}</ul> : <p>No prerequisite work.</p>}</div>
      <div><h4>Unblocks</h4>{unblocks.length ? <ul>{unblocks.map((edge) => <li key={edge.id}><span><b>→</b><strong>{edge.dependentTask.title}</strong><small>{dependencyTypeLabel[edge.type]}</small></span>{!readOnly ? <button type="button" onClick={() => void remove(edge)} disabled={saving} aria-label={`Remove dependency for ${edge.dependentTask.title}`}>×</button> : null}</li>)}</ul> : <p>No downstream work.</p>}</div>
    </div>
    {!readOnly ? <div className="task-dependencies__composer"><label><span>Add prerequisite</span><select value={selectedId} onChange={(event) => setSelectedId(event.target.value)} disabled={saving}><option value="">Choose a task…</option>{candidates.map((task) => <option key={task.id} value={task.id}>{task.title}</option>)}</select></label><label><span>Relationship</span><select value={type} onChange={(event) => setType(event.target.value as TaskDependencyType)} disabled={saving}>{Object.entries(dependencyTypeLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button type="button" className="secondary-button" onClick={() => void add()} disabled={!selectedId || saving}>{saving ? 'Saving…' : 'Add dependency'}</button></div> : null}
