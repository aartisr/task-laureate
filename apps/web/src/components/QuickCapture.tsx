...
export function QuickCapture({ triggerVariant = 'sidebar' }: QuickCaptureProps) {
  ...
  const dialog = <div className="quick-capture" role="presentation">
    ...
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="quick-capture-title" aria-describedby="quick-capture-description" className="panel quick-capture__dialog" onKeyDown={trapFocus}>
      <header className="quick-capture__header"><div><p className="eyebrow">Capture without context switching</p><h2 id="quick-capture-title">What just came to mind?</h2><p id="quick-capture-description">Save it now. We’ll organize it into your Inbox without interrupting your flow.</p></div><button type="button" className="quick-capture__close" onClick={close} aria-label="Close quick capture"><AppIcon name="close" /></button></header>
      ...
      <div className="quick-capture__body">
        ...
        <aside className="quick-capture__insight" aria-label="Capture details">{parsed.title ? <section className="quick-capture__preview" aria-label="Capture preview"><div className="quick-capture__preview-heading"><span aria-hidden="true"><AppIcon name="spark" /></span><div><strong>Ready to save</strong><p>{parsed.confidence === 'high' ? 'Details recognized automatically.' : `A clear ${destinationLabel} item will be created.`}</p></div></div><p className="quick-capture__title">{parsed.title}</p><div className="quick-capture__chips"><span>{destinationLabel}</span>{parsed.estimateMinutes ? <span>{parsed.estimateMinutes} min</span> : null}{parsed.scheduledStartAt ? <span>Tomorrow</span> : null}{parsed.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div></section> : <div className="quick-capture__rest-assurance"><span aria-hidden="true"><AppIcon name="spark" /></span><div><strong>Your flow is protected</strong><p>Nothing is lost—captures are saved locally first, then synced when possible.</p></div></div>}</aside>
      </div>
      ...
      <footer className="quick-capture__actions"><button type="button" className="primary-button" disabled={!parsed.title || isSaving} onClick={() => void save()}>{isSaving ? 'Saving…' : `Save to ${destinationLabel}`} <AppIcon name="arrow-right" /></button><button type="button" className="quick-capture__dismiss" disabled={isSaving} onClick={close}>Keep working</button></footer>
    </div>
  </div>;
