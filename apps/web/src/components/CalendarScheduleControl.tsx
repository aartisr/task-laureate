...
export function canScheduleCalendarBlock(input: { startsAt: string; selectedCalendar?: unknown; existingBlock?: { calendar_id: string } | null }) {
  return Boolean(input.startsAt && (input.existingBlock?.calendar_id || input.selectedCalendar));
}
...
export function CalendarScheduleControl({ task, scheduledStartAt, estimateMinutes, existingBlock, onScheduled }: { task: TodoItem; scheduledStartAt?: string | null; estimateMinutes?: number | null; existingBlock?: { calendar_id: string; starts_at: string; duration_minutes: number; external_event_url?: string | null; sync_state?: string; last_reconciled_at?: string | null } | null; onScheduled?: () => void | Promise<void> }) {
  ...
  const targetCalendarId = existingBlock?.calendar_id ?? calendarId;
  ...
  return <section className="calendar-schedule-control" aria-labelledby={`calendar-schedule-${task.id}`}>
    ...
    <div className="calendar-schedule-control__actions">
      ...
      <button className={existingBlock && !presentation.needsNewTime ? 'secondary-button' : 'primary-button'} type="button" disabled={busy || !canScheduleCalendarBlock({ startsAt, selectedCalendar, existingBlock })} onClick={() => void schedule()}>{busy ? 'Confirming…' : presentation.scheduleLabel}</button>
