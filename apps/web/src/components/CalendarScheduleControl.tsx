...
export function CalendarScheduleControl({ task, scheduledStartAt, estimateMinutes, existingBlock, onScheduled }: { task: TodoItem; scheduledStartAt?: string | null; estimateMinutes?: number | null; existingBlock?: { calendar_id: string; starts_at: string; duration_minutes: number; external_event_url?: string | null; sync_state?: string; last_reconciled_at?: string | null } | null; onScheduled?: () => void | Promise<void> }) {
  ...
  const sync = async () => { if (!status?.connectionId || !existingBlock) return; setBusy(true); setNotice(''); try { const result = await reconcileCalendar(task.id, status.connectionId, existingBlock.calendar_id); if (result.block?.sync_state === 'removed_external') setStartsAt(localDateTime()); else if (result.block?.starts_at) setStartsAt(localDateTime(result.block.starts_at)); if (result.block?.duration_minutes) setDuration(result.block.duration_minutes); await onScheduled?.(); setCheckedAt(new Date().toISOString()); setNotice(result.block?.sync_state === 'removed_external' ? 'Google Calendar confirms this block was removed.' : 'You are in sync. The confirmed Google time and duration are now shown.'); } catch (error) { setNotice(messageFor(error)); } finally { setBusy(false); } };
