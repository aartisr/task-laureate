...
export function CalendarScheduleControl({ task, scheduledStartAt, estimateMinutes, existingBlock, onScheduled }: { task: TodoItem; scheduledStartAt?: string | null; estimateMinutes?: number | null; existingBlock?: { calendar_id: string; starts_at: string; duration_minutes: number; external_event_url?: string | null; sync_state?: string; last_reconciled_at?: string | null } | null; onScheduled?: () => void }) {
  ...
  const sync = async () => { if (!status?.connectionId || !existingBlock) return; setBusy(true); setNotice(''); try { await reconcileCalendar(status.connectionId, existingBlock.calendar_id); setCheckedAt(new Date().toISOString()); setNotice('You are in sync. Any time changes from Google Calendar are now reflected in this task.'); onScheduled?.(); } catch (error) { setNotice(messageFor(error)); } finally { setBusy(false); } };
  ...
  return <section className="calendar-schedule-control" aria-labelledby={`calendar-schedule-${task.id}`}>
    <div>
      ...
      <h3 id={`calendar-schedule-${task.id}`}>{existingBlock ? 'This task has protected time' : 'Choose a time you can protect'}</h3>
      <p>{existingBlock?.sync_state === 'removed_external' ? 'This block was removed in Google Calendar. The task is safe; choose a new time whenever you are ready.' : existingBlock ? 'Open the exact event in Google Calendar, make any change there, then return here to confirm everything matches.' : 'A block is created only after Google Calendar confirms it. You can update or remove it here anytime.'}</p>
    </div>
    ...
    <div className="calendar-schedule-control__form"><label className="field"><span>Start</span><input type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} disabled={busy} /></label><label className="field"><span>Duration</span><select value={duration} onChange={(event) => setDuration(Number(event.target.value))} disabled={busy}>{[5, 10, 15, 30, 45, 60, 90, 120].map((minutes) => <option key={minutes} value={minutes}>{minutes < 60 ? `${minutes} minutes` : `${minutes / 60} hour${minutes === 60 ? '' : 's'}`}</option>)}</select></label><label className="field"><span>Calendar</span><select value={calendarId} onChange={(event) => setCalendarId(event.target.value)} disabled={busy || !!existingBlock}>{status.calendars?.map((calendar) => <option key={calendar.id} value={calendar.id}>{calendar.summary}{calendar.primary ? ' · Primary' : ''}</option>)}</select>{existingBlock ? <small>Remove this block before choosing a different calendar.</small> : null}</label></div>
    <div className="calendar-schedule-control__actions">
      {existingBlock ? <a className="primary-button" href={googleCalendarUrl} target="_blank" rel="noreferrer">Open in Google Calendar <span aria-hidden="true">↗</span></a> : null}
      <button className={existingBlock ? 'secondary-button' : 'primary-button'} type="button" disabled={busy || !startsAt || !selectedCalendar} onClick={() => void schedule()}>{busy ? 'Confirming…' : existingBlock ? 'Update this block' : 'Add to calendar'}</button>
      {existingBlock ? <button className="secondary-button" type="button" disabled={busy} onClick={() => void sync()}>{busy ? 'Checking…' : 'I made a change — check now'}</button> : null}
      {existingBlock ? <button className="secondary-button" type="button" disabled={busy} onClick={() => void remove()}>Remove block</button> : null}
