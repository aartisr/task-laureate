...
export function SharedWithMePage() {
  ...
  return <section className="shared-with-me-page" aria-labelledby="shared-with-me-title">
    ...
    {enabled && !isLoading && !error && data.length === 0 ? <div className="shared-with-me-page__empty"><span aria-hidden="true"><AppIcon name="share" /></span><h2>Nothing shared with you yet</h2><p>When someone grants you access, it will appear here—separate from your own Lists.</p></div> : null}
    {data.length ? <div className="shared-resource-grid">{data.map((resource) => <article className="shared-resource-card" key={`${resource.resourceType}:${resource.resourceId}`}>
      ...
      <footer><span>Shared access</span><button type="button" onClick={() => resource.resourceType === 'list' ? navigate({ to: '/lists/$listId', params: { listId: resource.resourceId } }) : navigate({ to: '/tasks' })}>Open {resource.resourceType === 'list' ? 'List' : 'Task'} <AppIcon name="arrow-right" /></button></footer>
    </article>)}</div> : null}
  </section>;
}
