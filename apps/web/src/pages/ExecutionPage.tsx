...
export function ExecutionPage() {
  ...
  return <main className="page-stack execution-page" aria-label="Execution workspace">
    ...
    <section className="panel execution-page__ritual"><div className="panel-heading"><div><p className="eyebrow">One-minute plan</p><h2>What do you have right now?</h2><p>Set your capacity, then protect no more than three meaningful commitments.</p></div>{dailyPlan.closedAt ? <span className="status-pill">Day reflected</span> : null}</div>
      <div className="execution-page__controls" aria-label="Available capacity">{energyOptions.map((option) => <button key={option.value} type="button" className={dailyPlan.energyLevel === option.value ? 'primary-button' : 'secondary-button'} onClick={() => persistPlan({ energyLevel: option.value, closedAt: null })}>{option.label}</button>)}<label className="field"><span>Minutes available</span><input type="number" min="5" max="480" value={dailyPlan.availableMinutes} onChange={(event) => persistPlan({ availableMinutes: Math.max(5, Math.min(480, Number(event.target.value) || 5)), closedAt: null })} /></label></div>
      <div className="execution-page__capacity"><div><strong>{model.plannedMinutes} of {dailyPlan.availableMinutes} minutes intentionally planned</strong><span>{dailyPlan.taskIds.length}/3 commitments</span></div><div className="execution-page__meter" role="progressbar" aria-label="Planned capacity" aria-valuemin={0} aria-valuemax={dailyPlan.availableMinutes} aria-valuenow={model.plannedMinutes}><i style={{ width: `${capacityPercent}%` }} /></div><p>{capacityPercent > 100 ? 'Your plan is over capacity—release or defer one task.' : capacityPercent >= 85 ? 'You are protecting a full, realistic window.' : 'You still have room only if it feels genuinely useful.'}</p></div>
    </section>
    ...
    <section className="panel"><div className="panel-heading"><div><h2>Today’s commitments</h2><p>A small promise is easier to keep than an endless list.</p></div></div>{model.committed.length ? <div className="card-list">{model.committed.map((task) => <ExecutionCard key={task.id} task={task} reason={`${planning.data?.[task.id]?.estimateMinutes ?? 'Unestimated'} min · ${planning.data?.[task.id]?.energyLevel ?? 'clarify energy'}`} isCommitted canCommit={false} isPending={mutations.completeTask.isPending || mutations.updateTask.isPending} onCommit={commit} onRelease={release} onComplete={complete} onSnooze={snooze} />)}</div> : <p className="empty-state">No commitments yet. Pick the recommendation above only if you are ready to protect time for it.</p>}</section>

    <div className="execution-page__grid"><TaskSection title="Focus block" description="Deep work that fits this window." items={model.focus.slice(0, 4)} committed={dailyPlan.taskIds} canCommit={dailyPlan.taskIds.length < 3} onCommit={commit} /><TaskSection title="Quick wins" description="Finishable in ten minutes or less." items={model.quick.slice(0, 4)} committed={dailyPlan.taskIds} canCommit={dailyPlan.taskIds.length < 3} onCommit={commit} /></div>
    <TaskSection title="Clarify before committing" description="These tasks need a duration or energy level before they belong in today’s plan." items={model.review.map((task) => ({ task, reasons: ['Add a time estimate and energy level first.'] })).slice(0, 8)} committed={dailyPlan.taskIds} canCommit={dailyPlan.taskIds.length < 3} onCommit={commit} />

    <section className="panel execution-page__shutdown"><div className="panel-heading"><div><p className="eyebrow">Close the loop</p><h2>Leave a useful note for tomorrow.</h2><p>One sentence is enough. This is reflection, not a scorecard.</p></div></div><textarea value={reflection} onChange={(event) => setReflection(event.target.value)} placeholder="What helped, what got in the way, or what should tomorrow know?" rows={3} /><div className="button-row"><button type="button" className="primary-button" onClick={closeDay}>{dailyPlan.closedAt ? 'Update reflection' : 'Close today gently'}</button>{dailyPlan.closedAt ? <span role="status">Reflected {new Date(dailyPlan.closedAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</span> : null}</div></section>
  </main>;
}
...
function TaskSection({ title, description, items, committed, canCommit, onCommit }: { title: string; description: string; items: Array<{ task: TaskWithList; reasons: string[] }>; committed: string[]; canCommit: boolean; onCommit: (id: string) => void }) {
  return <section className="panel"><div className="panel-heading"><div><h2>{title}</h2><p>{description}</p></div></div>{items.length ? <div className="card-list">{items.map(({ task, reasons }) => <article className="data-card" key={task.id}><div className="data-card__content"><strong>{task.title}</strong><p>{task.listTitle} · {reasons.join(' ')}</p></div><div className="button-row">{committed.includes(task.id) ? <span className="status-pill">In today</span> : <button className="primary-button" type="button" disabled={!canCommit} onClick={() => onCommit(task.id)}>{canCommit ? 'Add to today' : 'Today is full'}</button>}<Link className="secondary-button" to="/lists/$listId/tasks/$taskId" params={{ listId: task.listId, taskId: task.id }}>Details</Link></div></article>)}</div> : <p className="empty-state">Nothing fits this view yet. Try a different window or clarify a task.</p>}</section>;
}
