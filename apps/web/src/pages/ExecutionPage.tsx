...
export function ExecutionPage() {
  ...
  return <main className="page-stack execution-page" aria-label="Execution workspace">
    ...
    <section className="panel execution-page__commitments"><div className="panel-heading"><div><p className="eyebrow">Protected time</p><h2>Today’s commitments</h2><p>A small promise is easier to keep than an endless list. Keep only what you are prepared to protect.</p></div><span className="execution-page__commitment-count">{model.committed.length}/3 committed</span></div>{model.committed.length ? <div className="card-list execution-page__commitment-list">{model.committed.map((task) => <ExecutionCard key={task.id} task={task} reason={`${planning.data?.[task.id]?.estimateMinutes ?? 'Unestimated'} min · ${planning.data?.[task.id]?.energyLevel ?? 'clarify energy'}`} isCommitted canCommit={false} isPending={mutations.completeTask.isPending || mutations.updateTask.isPending} onCommit={commit} onRelease={release} onComplete={complete} onSnooze={snooze} />)}</div> : <p className="empty-state execution-page__commitments-empty">No commitments yet. Choose the recommendation above only when you are ready to protect time for it.</p>}</section>
