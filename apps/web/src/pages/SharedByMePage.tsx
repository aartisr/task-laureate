...
export function SharedByMePage() {
  ...
  return <section className="shared-with-me-page" aria-labelledby="shared-by-me-title">
    ...
    {enabled && !isLoading && !error && data.length === 0 ? <div className="shared-with-me-page__empty"><span aria-hidden="true">✦</span><h2>No Lists shared by you yet</h2><p>Share a List from its Share panel. Collaborators and pending invitations will appear here.</p></div> : null}
    {data.length ? <div className="shared-resource-grid">{data.map((list) => <article className="shared-resource-card" key={list.listId}>
      ...
      <footer><span>{describeSharing(list.collaboratorCount, list.pendingInvitationCount)}</span><button type="button" onClick={() => navigate({ to: '/lists/$listId', params: { listId: list.listId } })}>Open List <span aria-hidden="true">→</span></button></footer>
    </article>)}</div> : null}
  </section>;
}
